...
export const BannersMobileInfoAdmin = () => {
  ...
  useEffect(() => {
    axios.get(`${Constants.API_URL}banners/mobile/all`).then((response) => {
  ...
  function handleUpdateBannerActive(e: any, banner_id: number) {
    const options = {
      method: 'PATCH',
      url: `${Constants.API_URL}banners/mobile/${banner_id}`,
      headers: {
  ...
  function handleDeleteBannerConfirm() {
    axios.delete(`${Constants.API_URL}banners/mobile/${bannerToDeleteId}`, {
      headers: {
        'Bearer': `${userState.data?.access_token}`
      }
    }).then(res => {
        ...
        <Header>
          <HStack justifyContent="space-between">
            ...
            <Box>
              <Button
                as={Link}
                to="/admin/banners/mobile/create"
                colorScheme="green"
                size={"lg"}
                  ...
                  <Box position={"absolute"} top={4} right={4}>
                    <HStack>
                      <Tooltip label="Editar" aria-label="Editar">
                        <IconButton
                        as={Link}
                        to={`/admin/banners/mobile/${banner.id}/edit`}
